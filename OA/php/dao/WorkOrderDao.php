...
class WorkOrderDao{
	...
	function getWorkOrderList($searchParam,$operateFlag){
		...
		if($operateFlag == 'vacationList'){	//请假/出差、加班
			$sql .= 'SELECT VR.ID,VR.CREATE_TIME,VR.TYPE,VR.LEAVE_TYPE,VR.AUDIT_STATUS,u1.USER_NAME AS CURRENT_USER_NAME,u2.USER_NAME AS PRIVIOUS_USER_NAME ';
	 ...
	 */
	 function getWorkOrderById($id,$flag){
 		...
 		if($flag == 'leaveTrip' || $flag == 'overtimeRequest'){
		 	...
		 	$sql .= ',u1.USER_NAME AS CURRENT_USER_NAME,u2.USER_NAME AS PRIVIOUS_USER_NAME,u3.USER_NAME AS USER_NAME ';
		 	$sql .= 'FROM VACATION_RECORD VR LEFT JOIN USER_INFO u1 ON VR.CURRENT_USERID=u1.ID LEFT JOIN USER_INFO u2 ON VR.PREVIOUS_USERID=u2.ID LEFT JOIN USER_INFO u3 ON VR.USER_ID=u3.ID WHERE VR.ID=' . $id;
 		}else{
 			$sql .= 'SELECT FC.TYPE,FC.CREATE_TIME,FC.FORGET_TIME,FC.IN_TIME,FC.REMARK,FC.AUDIT_STATUS,FC.AUDIT_TIME,FC.AUDIT_REMARK';
 			$sql .= ',u1.USER_NAME AS CURRENT_USER_NAME,u2.USER_NAME AS PRIVIOUS_USER_NAME,u3.USER_NAME AS USER_NAME  ';
		 	$sql .= 'FROM FORGET_CARD FC LEFT JOIN USER_INFO u1 ON FC.CURRENT_USERID=u1.ID LEFT JOIN USER_INFO u2 ON FC.PREVIOUS_USERID=u2.ID LEFT JOIN USER_INFO u3 ON FC.USER_ID=u3.ID WHERE FC.ID=' . $id;
 		}
	 	$conn = new MysqlConn();
